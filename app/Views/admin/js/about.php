...
<script>
    ...
    function deleteAbout(id) {
        Swal.fire({
            heightAuto: false,
            title: 'Delete About/Homepage Content',
            text: 'Are you sure you want to permanently delete this content? This action cannot be undone.',
            icon: 'warning',
            showCancelButton: true,
            ...
            $.post("<?php echo site_url('admin/ajax/delete_about') ?>", { id: id }, function (response) {
                if (response.status == 1) {
                    tbl.ajax.reload(null, false);
                    Swal.fire({ icon: 'success', title: 'Deleted', text: response.message || 'Content deleted successfully.' });
                } else {
                    Swal.fire({ icon: 'error', title: 'Delete Failed', text: response.message || response.msg || 'Unable to delete the content.' });
                }
            }).fail(function (xhr) {
                var response = xhr.responseJSON || {};
